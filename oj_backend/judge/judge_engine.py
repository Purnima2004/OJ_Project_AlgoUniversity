...
from compiler.engine import CodeCompiler
...
class OnlineJudge:
    ...
    def _run_test_case(self, submission: Submission, test_case: TestCase) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._compare_output(result['output'], test_case.expected_output):
                print(f"DEBUG: Test case {test_case.id} PASSED - Marking as AC")
                return {
                    'status': 'AC',
                    'execution_time': result['execution_time'],
                    'memory_used': result.get('memory_used', 0),
                ...
                print(f"DEBUG: Test case {test_case.id} FAILED - Marking as WA")
                print(f"DEBUG: Expected: '{test_case.expected_output}'")
                print(f"DEBUG: Got: '{result['output']}'")
                return {
                    'status': 'WA',
                    'execution_time': result['execution_time'],
                    'memory_used': result.get('memory_used', 0),
    ...
    def _compare_output(self, actual: str, expected: str) -> bool:
        """Compare actual output with expected output with more flexible matching"""
        ...
        # Try more flexible comparison - split into lines and compare each line
        ...
        if len(actual_lines) != len(expected_lines):
            print(f"Match: FALSE (line count mismatch: {len(actual_lines)} vs {len(expected_lines)})")
            return False
        
        # Compare each line
        for i, (actual_line, expected_line) in enumerate(zip(actual_lines, expected_lines)):
            if actual_line != expected_line:
                print(f"Match: FALSE (line {i+1} mismatch: '{actual_line}' vs '{expected_line}')")
                return False
        
        print(f"Match: TRUE (flexible match)")
        return True
    ...
    def _determine_overall_status(self, results: List[Dict[str, Any]]) -> str:
        ...
        if not results:
            print("DEBUG: No test case results - marking as WA")
            return 'WA'
        
        # Check for serious errors first
        for result in results:
            if result['status'] in ['TLE', 'RE', 'CE', 'MLE']:
                print(f"DEBUG: Found serious error {result['status']} - marking submission as {result['status']}")
                return result['status']
        
        # Check if all test cases passed
        all_passed = all(result['status'] == 'AC' for result in results)
        overall_status = 'AC' if all_passed else 'WA'
        
        print(f"DEBUG: Overall status determination:")
        print(f"DEBUG: Test case results: {[r['status'] for r in results]}")
        print(f"DEBUG: All passed: {all_passed}")
        print(f"DEBUG: Final status: {overall_status}")
