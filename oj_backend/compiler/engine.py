...
class CodeCompiler:
    ...
    def _secure_run(self, cmd: list, input_data: str = None, 
                   timeout: int = TIME_LIMIT, working_dir: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Use provided working directory or default to temp_dir
            working_directory = working_dir if working_dir else self.temp_dir
            ...
            result = subprocess.run(
                ...
                cwd=working_directory  # Use the working directory parameter
            )
        ...
        try:
            ...
            for gpp_cmd in cpp_commands:
                try:
                    ...
                    # FIX: Use working_dir parameter to run from codes directory
                    compile_result = self._secure_run(compile_cmd, working_dir=str(codes_dir))
            ...
            # FIX: Run with working_dir parameter
            start_time = time.time()
            run_result = self._secure_run([str(executable)], input_data, working_dir=str(codes_dir))
    ...
    def _run_java(self, code_file: Path, java_class_name: str, 
        ...
        for javac_cmd in java_commands:
            try:
                ...
                # Run javac from the codes directory where the .java file is located
                compile_result = self._secure_run(compile_cmd, working_dir=str(Path(self.temp_dir) / "codes"))
        ...
        for java_cmd in java_commands:
            try:
                ...
                # Run java from the codes directory where the .class file is located
                run_result = self._secure_run(run_cmd, input_data, working_dir=str(Path(self.temp_dir) / "codes"))
                execution_time = time.time() - start_time
